...
volatile unsigned long water = 0;
...
volatile unsigned int PinState = 0;         // current state LDR
volatile unsigned int lastPinState = 0;     // previous state LDR
...
#if defined LDR_SENSOR && NODEID == 3 

 pinMode((LDR_PIN+14), INPUT);
...
void doMeasure() {       //main measure
 ...
 analogReference(INTERNAL);
    ...
    if (measure.light < 150) {
      
      measure.light = 0;
      
    } else if (measure.light >800) {
...
#endif   //EMD defined LDR_SENSOR && NODEID != 3

#if defined LDR_SENSOR && NODEID == 3

int status_rain = digitalRead(LDR_PIN+14);
...
if (status_rain == HIGH) {
    ...
    #ifdef DEBUG
       Serial.print("Status rain digital value:  ");
       delay(2);
       Serial.println(status_rain);
       delay(2);
...
#if defined LDR_SENSOR && NODEID == 3
int rain_as_ldr(void) {
 
 
 
  PinState = digitalRead((LDR_PIN+14));
  delay(2);
   ...
   if (PinState != lastPinState) {
      ...
      Serial.print("Last PinState:  ");
      delay (2);
      Serial.println(lastPinState);
      delay (2);
  
      #endif 
        if ((digitalRead(LDR_PIN+14)) == HIGH) {
...
#if defined LDR_SENSOR && NODEID == 3
  if (rainTimer.poll(20000)) {
  int status_rain2 = rain_as_ldr();
    #if defined DEBUG
       Serial.println();
       Serial.print("Last PinState function print - rain status:  ");
       delay(2);
       Serial.print((float)status_rain2/10,2);Serial.println(" %");
       delay(2);
   ...
   //for (byte i=0;i<10;i++){   // loop for to be sure that proper package can be received  WAIT_TIME_MULTIPLY
    ...
    if (sendTimer.poll((10000 * WAIT_TIME_MULTIPLY))) {// period sending set multiply by wait_time value ~ 3 sek procesing all and send 10 sek is a default multiply 1 time betwen sending measures change WAIT_TIME_MULTIPLY for multiply time 
              ...
              #endif
         if ( water == 1 && flag_pulse == 1) {
            //measure.pulse = measure.pulse + pulseCount;
...
//sleep mode without relays
     ...
     //send data if there are some
              ...
              rf12_sleep(RF12_SLEEP);
                ...
                delay(10);
                if (water == 1 && flag_pulse == 1) break;      // exit the for(), we're done receiving pulse according INT 1 interput pulse received
                if (water == 0 && flag_pulse == 0 ) continue;  // if nothing hapened  // for sleep mode || occurence == 1
...
//for pulse measure produce data
      #if defined PULSE_MEASURE && !defined BATTERY_ONLY
        #ifdef DEBUG
...
//THERE IS FOR BATTERY ONLY - save energy
  ...
  if (sendTimer.poll(10000)) {
    ...
    long sometime;
    if (WAIT_TIME_MULTIPLY >=1) sometime = 45000;
  ...
  delay(60);
    ...
    if (pulseCount == 1 || flag_pulse == 1) break;      // exit the for(), we're done receiving pulse according INT 1 interput pulse received
 ...
 unsigned long getvalueEEPROM=0;
   //cli();  //disable global interputs
    ...
    if (measure.pulse/10 > getvalueEEPROM/10){
      ...
      #ifdef DEBUG
      Serial.print("Current ulse value set from EEPROM: ");
      delay(2);
      Serial.println(measure.pulse);
      #endif
...
void onPulse() {
    ...
    //unsigned long interval = newBlink-lastBlink;
       ...
       //if ((long)(micros() -lastBlink)>=5000L) { // Sometimes we get interrupt on RISING
           ...
           if (pulseCount == 4100) {
            ...
            delay(2);
            Serial.println(test);
            Serial.print("Current measure: "); Serial.println(measure.pulse);
            delay(10);
          #endif
          }
          //lastBlink = micros(); 
